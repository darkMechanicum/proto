...
object LookupUtils {
    ...
    fun getBindHandle(method: Method, proxy: Any) = getBindHandle(method, method.declaringClass, proxy)

}

class CommonProxy(
    val handledClass: Class<*>,
    val nonHibernate: Boolean = false
) : InvocationHandler {
    ...
    fun getOriginal(pName: String): Any? =
        pNameToGetter[pName]?.let { indexedMethods[it] }?.invoke(bindProxy)

    fun setOriginal(pName: String, arg: Any?) =
        pNameToSetter[pName]?.let { indexedMethods[it] }?.invoke(bindProxy, arg)
    ...
    override fun invoke(proxy: Any, method: Method, args: Array<out Any?>?): Any? {
        ...
        if (method.isDefault) {
            mutHandle = LookupUtils.getBindHandle(method, proxy)
        }
